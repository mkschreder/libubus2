...
typedef void (*ubus_socket_msg_cb_t)(ubus_socket_t socket, uint32_t peer, struct blob_field *msg);  
...
struct ubus_socket_api {
	...
	int 	(*send)(ubus_socket_t ptr, int32_t peer, struct blob_field *msg); 
...
#define ubus_socket_send(sock, peer, msg) (*sock)->send(sock, peer, msg)
